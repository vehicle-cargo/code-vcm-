...
vehicle_df = pd.read_csv('dataset/kpca-kpca-data/main_vehicle_cargo_dataset/vehicle_3.csv', header=None)  # header=None自动添加列字段
cargo_df = pd.read_csv('dataset/kpca-kpca-data/main_vehicle_cargo_dataset/cargo_3.csv', header=None)
# 40*6
# print(vehicle_df)
# print(cargo_df)
...
for t in time:
    print('当前时间为t=', t)
    # 从数据集中读取车辆的数据-不够十辆
    vcm_change.appendVehicle(vehicle_df, vehicle_maxNum, vehicle_queue, t)
    # 从数据集中读取货物的数据
    vcm_change.appendCargo(cargo_df, cargo_maxNum, cargo_queue, t)
    # 获得车辆和货物的属性概率分布（先验概率）
    ...
    for vehicleKey, vehicleValue in vehicle_queue.items():
        ...
        w2 = 0.17
        w3 = 0.13
        w4 = 0.04 # [0.190757   0.18569212 0.11184538 0.02817734 0.20538436 0.2781438 ]
        w5 = 0.21
        w6 = 0.26
        # 对于每个货物来
        for cargoKey, cargoValue in cargo_queue.items():
            attrMatch = vcm_change.VCM(vehicleValue, cargoValue, w1, w2, w3, w4, w5, w6)
            if attrMatch == 0:  # 如果属性匹配度为0 则最终的匹配度为0
                finalMatch = 0
            else:
                # prior为先验概率-环境匹配度
                finalMatch = 0.47 * attrMatch + (0.53 * vehicle_prior[m]) / (1 - cargo_prior[n])
