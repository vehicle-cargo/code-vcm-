...
vehicle_df = pd.read_csv('../code(vcm)/dataset/d-vehicle2.csv', header=None)  # header=None自动添加列字段
cargo_df = pd.read_csv('../code(vcm)/dataset/d-cargo2.csv', header=None)
# 40*6
# print(vehicle_df)
# print(cargo_df)
...
for t in time:
    ...
    for vehicleKey, vehicleValue in vehicle_queue.items():
       ...
       #  [0.26684519 0.1316941  0.31269074 0.28876998]
        # 动态权重 出现几次-有几次没有匹配成功
        # for v in range(0, len(faile_vehicle_time)):
        #     if vehicleKey in faile_vehicle_time[v]:
        #         q = q + 1
        #
        # if q > 0:
        #     if (w4 + w4 * q) > 0.5:
