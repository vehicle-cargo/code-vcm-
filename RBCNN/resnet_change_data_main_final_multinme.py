...
class ResNet(ModuleWrapper):
    ...
    def __init__(self,
        ...
        if self.include_top:
            ...
            #self.fc1 = nn.Linear(128 * block.expansion, 256, bias=True)  # 对应全连接层输出的分类个数
            self.fc2 = BBBLinear(256, 1, bias=True, priors=self.priors)
...
#  准备数据集
data = pd.read_csv('multime_data/kpca_vcm.csv')
data = np.array(data)
...
target = torch.tensor([0.37]).to(device)
