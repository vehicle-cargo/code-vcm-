...
class ResNet(ModuleWrapper):
    ...
    def __init__(self,
        ...
        if self.include_top:
            ...
            self.fc1 = BBBLinear(64 * block.expansion, 1, bias=True,priors=self.priors)  # 对应全连接层输出的分类个数
            #self.fc2 = BBBLinear(64, 1, bias=True, priors=self.priors)
...
# 最小-最大归一化
# main_data = [float(line.strip()) for line in main_data]
# min_val = np.min(main_data)
# max_val = np.max(main_data)
# normalized_data = (main_data - min_val) / (max_val - min_val)
# 训练集
...
optimizer = optim.Adam(net.parameters(), lr=0.00002)  # 0.00002
...
net.eval()
with torch.no_grad():
    output = net(new_test_data)
    print("Predicted output:", output.cpu().numpy())
