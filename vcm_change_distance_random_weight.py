...
type_match_list = [['低栏车', '日用百货', 0.8], ['低栏车', '特殊货物', 0.3], ['低栏车', '机器零件', 1],
                   ...
                   ['通风箱车', '生鲜果蔬', 1],['通风箱车', '砂石散货', 0.5], ['通风箱车', '五金机械', 0.8],
...
vehicle_df = pd.read_csv('dataset/kpca-kpca-data/main_vehicle_cargo_dataset/deng/v_deng4.csv', header=None)  # header=None自动添加列字段
cargo_df = pd.read_csv('dataset/kpca-kpca-data/main_vehicle_cargo_dataset/deng/c_deng4.csv', header=None)
# 40*6
# print(vehicle_df)
# print(cargo_df)
    ...
    for vehicleKey, vehicleValue in vehicle_queue.items():
        ...
        for cargoKey, cargoValue in cargo_queue.items():
            ...
            if attrMatch == 0:  # 如果属性匹配度为0 则最终的匹配度为0
                ...
                # prior为先验概率-环境匹配度
                finalMatch = 0.5 * attrMatch + (0.5 * vehicle_prior[m]) / (1 - cargo_prior[n])
