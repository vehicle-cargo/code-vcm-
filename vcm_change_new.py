...
def VCM(a, b, real_data_dict, w1=0.16, w2=0.09, w3=0.47, w4=0.28, w5=0.2, w6=0.13):
    ...
    # 体积或质量匹配度
    gMatch, vMatch, lMatch = getLoadabilityMatch(a[1], a[2], a[3], b[1], b[2], b[3])
    LoadabilityMatch = w2 * gMatch + w3 * vMatch + w4 * lMatch
    # 路径匹配度
    flowMatch = getFlowMatch(a[9], b[9], real_data_dict)
    # 时间匹配度
    timeMatchs = timeMatch(a[8][0], a[8][1], b[8][0], b[8][1])
    # 属性匹配度
    attrMatch = w1 * typeMatch + LoadabilityMatch + w5 * flowMatch + w6 * timeMatchs
    # 对于属性匹配度，保留两位小数
    attrMatch = round(attrMatch, 2)
    # 如果任何一个属性匹配度为为0 则不满足匹配的条件
    if typeMatch == 0 or LoadabilityMatch == 0 or flowMatch == 0 or timeMatchs == 0:
        attrMatch = 0
    return attrMatch
