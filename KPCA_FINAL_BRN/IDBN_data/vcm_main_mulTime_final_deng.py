...
time = [0, 1, 2, 3]
# 从数据集中读取数据
vehicle_df = pd.read_csv('deng/v13.csv', header=None)
cargo_df = pd.read_csv('deng/c13.csv', header=None)
...
for t in time:
    ...
    for vehicleKey, vehicleValue in vehicle_queue.items():
        ...
        #         w3 = w3 - (0.1 * q) / 3
        for cargoKey, cargoValue in cargo_queue.items():
            attrMatch = vcm.rePaperVCM(vehicleValue, cargoValue, real_data_dict, w1, w2, w3, w4)
            if attrMatch == 0:
                finalMatch = 0
            else:
