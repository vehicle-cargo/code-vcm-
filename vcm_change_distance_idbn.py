...
vehicle_df = pd.read_csv('new_dataset/kpca_final_data/idb/v_idbn4.csv', header=None)  # header=None自动添加列字段
cargo_df = pd.read_csv('new_dataset/kpca_final_data/idb/c_idbn4.csv', header=None)
# 40*6
# print(vehicle_df)
# print(cargo_df)
...
for t in time:
    ...
    for vehicleKey, vehicleValue in vehicle_queue.items():
        ...
        w6 = 0.17
        q = 0
        #  [0.26684519 0.1316941  0.31269074 0.28876998]
        # 动态权重 出现几次-有几次没有匹配成功
    ...
    # 匹配失败时会考虑时间动态权重
    fail_vehicle_time.append(faile_vehilce_num)
    fail_cargo_time.append(faile_cargo_num)
...
print('匹配失败的车辆(t=0,1,2,3,4):', fail_vehicle_time)
print('匹配失败的货物(t=0,1,2,3,4):', fail_cargo_time)
